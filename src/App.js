...
function getRouterByRoutes(routes) {
  ...
  const renderRoutes = (routes, parentPath) => {
    Array.isArray(routes) && routes.forEach((route) => {
      ...
      if (redirect) renderedRoutesList.push(<Redirect key={`${parentPath}${path}`} exact from={path} to={`${parentPath}${redirect}`} />)
      if (component) {
      ...
      if (Array.isArray(children) && children.length > 0) renderRoutes(children, path)
    });
  }
  renderRoutes(routes, '')
